...
export async function handleConfigureRoleByZone(req: Request, res: Response) {
    try {
        ...
        if (!guild) {
            return res.send(
                respondWithMessageInEmbed(`Configure Role Failed`, `Guild not initialized; please run \`/${INIT.name}\``, Status.failure)
            );
        }
        ...
        if (e instanceof TypeError) return res.send(respondWithMessageInEmbed('Error', 'Invalid Request structure', Status.failure));
        else return res.send(respondWithMessageInEmbed('Configure Role Failed', `${e}`, Status.failure));
    }
}
